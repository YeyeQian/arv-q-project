...
*
* Arguments:   - int32_t r[256]: pointer to input/output vector of elements
*                                of Zq
*
* Requirements: -VLEN=256
**************************************************/
void ntt_cg_custom_zeta_compact(int32_t r[N]){
    int32_t r_temp0[256];
    unsigned int i, j;
    int32_t* r_ptr0 = NULL;
    int32_t* r_ptr1 = NULL;
    ...
    v_zetas=vle32_v_i32m1(&zetas_pos[1],1);
    for(i=0;i<8;i++){
    ...
    v_zetas=vle32_v_i32m1(&zetas_pos[2],2);
    for(i=0;i<8;i++){
        v_in0=vle32_v_i32m2(r_ptr0,16);
        v_in1=vle32_v_i32m2(r_ptr1,16);
        r_ptr0+=16;
    ...
    v_zetas=vle32_v_i32m1(&zetas_pos[4],4);
    for(i=0;i<8;i++){
        v_in0=vle32_v_i32m2(r_ptr0,16);
        v_in1=vle32_v_i32m2(r_ptr1,16);
        r_ptr0+=16;
    ...
    v_zetas=vle32_v_i32m1(&zetas_pos[8],8);
    for(i=0;i<8;i++){
        v_in0=vle32_v_i32m2(r_ptr0,16);
        v_in1=vle32_v_i32m2(r_ptr1,16);
        r_ptr0+=16;
    ...
    for(i=0;i<2;i++){
        ...
        v_zetas=vle32_v_i32m1(&zetas_pos[16+offset],8);
        for(j=0;j<8;j++){
    ...
    for(i=0;i<4;i++){
        ...
        r_ptr2=r+(offset<<1);
        v_zetas=vle32_v_i32m1(&zetas_pos[32+offset],8);
        for(j=0;j<4;j++){
            v_in2=vle32_v_i32m1(r_ptr0,8);
            v_in3=vle32_v_i32m1(r_ptr1,8);
            r_ptr0+=32;
    ...
    for(i=0;i<8;i++){
        ...
        r_ptr1=&r[128+offset];
        r_ptr2=r_temp0+(offset<<1);
        v_zetas=vle32_v_i32m1(&zetas_pos[64+offset],8);
        for(j=0;j<2;j++){
            v_in2=vle32_v_i32m1(r_ptr0,8);
            v_in3=vle32_v_i32m1(r_ptr1,8);
            r_ptr0+=64;
    ...
    for(i=0;i<16;i++){
        ...
        r_ptr1=&r_temp0[128+offset];
        r_ptr2=r+(offset<<1);
        v_zetas=vle32_v_i32m1(&zetas_pos[128+offset],8);
        v_in2=vle32_v_i32m1(r_ptr0,8);
        v_in3=vle32_v_i32m1(r_ptr1,8);
        v_out1=vbutterfly_ct_vvm_i32m2(v_in2,v_in3,v_zetas);
        vse32_v_i32m2(r_ptr2,v_out1,16);
...
* Requirements: -VLEN=256
**************************************************/
void ntt_cg_custom_reorder(int32_t r[N]){
    int32_t r_temp0[256];
    unsigned int i, j;
    int32_t* r_ptr0 = NULL;
    int32_t* r_ptr1 = NULL;
...
* Requirements: -VLEN=256
**************************************************/
void invntt_cg_custom_zeta_compact(int32_t r[N]){
    int32_t r_temp0[256];
    unsigned int i;
    int32_t* r_ptr0 = NULL;
    int32_t* r_ptr1 = NULL;
    int32_t* r_ptr2 = NULL;
    int32_t* zeta_ptr = zetas_inv_in_order;
...
* Requirements: -VLEN=256
**************************************************/
void invntt_cg_custom_reorder(int32_t r[N]){
    int32_t r_temp0[256];
    unsigned int i;
    int32_t* r_ptr0 = NULL;
    int32_t* r_ptr1 = NULL;
...
void ntt_cg_custom_zeta_compact(int32_t r[N]){
    ...
    v_zetas=vle32_v_i32m1(&zetas_pos[16],16);
    for(i=0;i<4;i++){
        v_in0=vle32_v_i32m2(r_ptr0,32);
        v_in1=vle32_v_i32m2(r_ptr1,32);
        r_ptr0+=32;
    ...
    for(i=0;i<2;i++){
        ...
        r_ptr1=&r_temp0[128+offset];
        r_ptr2=r+(offset<<1);
        v_zetas=vle32_v_i32m1(&zetas_pos[32+offset],16);
        for(j=0;j<4;j++){
    ...
    for(i=0;i<4;i++){
        ...
        r_ptr1=&r[128+offset];
        r_ptr2=r_temp0+(offset<<1);
        v_zetas=vle32_v_i32m1(&zetas_pos[64+offset],16);
        for(j=0;j<2;j++){
            v_in2=vle32_v_i32m1(r_ptr0,16);
            v_in3=vle32_v_i32m1(r_ptr1,16);
            r_ptr0+=64;
    ...
    for(i=0;i<8;i++){
        ...
        r_ptr1=&r_temp0[128+offset];
        r_ptr2=r+(offset<<1);
        v_zetas=vle32_v_i32m1(&zetas_pos[128+offset],16);
        v_in2=vle32_v_i32m1(r_ptr0,16);
        v_in3=vle32_v_i32m1(r_ptr1,16);
        v_out1=vbutterfly_ct_vvm_i32m2(v_in2,v_in3,v_zetas);
        vse32_v_i32m2(r_ptr2,v_out1,32);
...
* Requirements: -VLEN=512
**************************************************/
void ntt_cg_custom_reorder_asm(int32_t r[N]){
    size_t load_zeta_vl;
    size_t avl=32;//m2
    ...
    :
    : [zeta_vl]"r"(load_zeta_vl), [zeta_base]"r"(&zetas_pos[1]), [avl]"r"(avl),
...
* Requirements: -VLEN=512
**************************************************/
void invntt_cg_custom_reorder_asm(int32_t r[N]){
    size_t load_zeta_vl;
    size_t avl;//m1
    ...
    :
    : [zeta_vl]"r"(load_zeta_vl), [zeta_base_0]"r"(&zetas_inv_in_order[0]), [top_base_0]"r"(&r[0]), [btm_base_0]"r"(&r[128]),
      [zeta_base_1]"r"(&zetas_inv_in_order[16]), [top_base_1]"r"(&r[16]), [btm_base_1]"r"(&r[144]),
      [zeta_base_2]"r"(&zetas_inv_in_order[32]), [top_base_2]"r"(&r[32]), [btm_base_2]"r"(&r[160]),
      [zeta_base_3]"r"(&zetas_inv_in_order[48]), [top_base_3]"r"(&r[48]), [btm_base_3]"r"(&r[176]),
      [zeta_base_4]"r"(&zetas_inv_in_order[64]), [top_base_4]"r"(&r[64]), [btm_base_4]"r"(&r[192]),
      [zeta_base_5]"r"(&zetas_inv_in_order[80]), [top_base_5]"r"(&r[80]), [btm_base_5]"r"(&r[208]),
      [zeta_base_6]"r"(&zetas_inv_in_order[96]), [top_base_6]"r"(&r[96]), [btm_base_6]"r"(&r[224]),
      [zeta_base_7]"r"(&zetas_inv_in_order[112]), [top_base_7]"r"(&r[112]), [btm_base_7]"r"(&r[240])
  );
...
void ntt_cg_custom_reorder(int32_t r[N]){
    ...
    v_zetas=vle32_v_i32m1(&zetas_pos[32],32);
    v_in0=vle32_v_i32m4(r_ptr0,128);
    v_in1=vle32_v_i32m4(r_ptr1,128);
    vsetvl_e32m4_wrapper(128);
    v_out0=vbutterfly_ct_vvm_i32m8(v_in0,v_in1,v_zetas);
    ...
    for(i=0;i<2;i++){
        ...
        r_ptr1=&r[128+offset];
        r_ptr2=r_temp0+(offset<<1);
        v_zetas=vle32_v_i32m1(&zetas_pos[64+offset],32);
        for(j=0;j<2;j++){
    ...
    for(i=0;i<4;i++){
        ...
        v_zetas=vle32_v_i32m1(&zetas_pos[128+offset],32);
        v_in2=vle32_v_i32m1(r_ptr0,32);
        v_in3=vle32_v_i32m1(r_ptr1,32);
        v_out1=vbutterfly_ct_vvm_i32m2(v_in2,v_in3,v_zetas);
