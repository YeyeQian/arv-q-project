...
int crypto_kem_keypair_bf(unsigned char *pk, unsigned char *sk)
{
  ...
  indcpa_kem_keypair_bf(pk, sk); // sk[0:SABER_INDCPA_SECRETKEYBYTES-1] <-- sk
  for (i = 0; i < SABER_INDCPA_PUBLICKEYBYTES; i++)
    sk[i + SABER_INDCPA_SECRETKEYBYTES] = pk[i]; // sk[SABER_INDCPA_SECRETKEYBYTES:SABER_INDCPA_SECRETKEYBYTES+SABER_INDCPA_SECRETKEYBYTES-1] <-- pk
...
int crypto_kem_enc_bf(unsigned char *c, unsigned char *k, const unsigned char *pk)
{
  ...
  sha3_512(kr, buf, 64);               // kr[0:63] <-- Hash(buf[0:63]);
                                       // K^ <-- kr[0:31]
                                       // noiseseed (r) <-- kr[32:63];
  indcpa_kem_enc_bf(buf, kr + 32, pk, c); // buf[0:31] contains message; kr[32:63] contains randomness r;
...
int crypto_kem_dec_bf(unsigned char *k, const unsigned char *c, const unsigned char *sk)
{
  int i, fail;
  unsigned char cmp[SABER_BYTES_CCA_DEC];
  unsigned char buf[64];
  ...
  indcpa_kem_dec_bf(sk, c, buf); // buf[0:31] <-- message
  ...
  indcpa_kem_enc_bf(buf, kr + 32, pk, cmp);
