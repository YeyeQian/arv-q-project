...
void poly_mul_acc_plain(const uint16_t a[SABER_N], const uint16_t b[SABER_N], uint16_t res[SABER_N]) {
    ...
    for (int i = 0; i < SABER_N; i++) {
        unsigned short temp_acc_1 = 0;
        short temp_acc_2 = 0;
        for (int j = 0; j <= i; j++) {
            temp_acc_1 = (temp_acc_1 + ((unsigned short)a[j] * (unsigned short)b[i - j]) % q) % q;
        }
        for (int j = i + 1; j < SABER_N; j++) {
            temp_acc_2 = (temp_acc_2 + ((unsigned short)a[j] *(unsigned short) b[SABER_N - j + i]) % q) % q;
        }
        unsigned short c = temp_acc_1 - temp_acc_2;
        if (c < 0) {
            c += q;
        }
        res[i] = (uint16_t)(res[i]+c)%q;
    }
}

void poly_mul_acc_bf(uint32_t a[SABER_N], uint32_t b[SABER_N], uint32_t res[SABER_N]) {
    for (int i = 0; i < SABER_N; i++) {
        res[i] = mod_add(res[i], montgomery_reduce(a[i], b[i]));
    }
}