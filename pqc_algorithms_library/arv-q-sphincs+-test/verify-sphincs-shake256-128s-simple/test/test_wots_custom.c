...
bool wots_custom_profiling()
{
    ...
    printf("sphincs-shake256-128s-simple wots_gen_pk_custom takes %ld cycles in HW/SW co-design\n", end - start);

    printf("begin wots_sign_custom for profiling\n");
    start = read_cycle();
    wots_sign_custom(sig, m, seed, pub_seed, addr);
    end = read_cycle();
    printf("sphincs-shake256-128s-simple wots_sign_custom takes %ld cycles in HW/SW co-design\n", end - start);

    printf("begin wots_pk_from_sig_custom for profiling\n");
    start = read_cycle();
    wots_pk_from_sig_custom(pk2, sig, m, pub_seed, addr);
    end = read_cycle();
    printf("sphincs-shake256-128s-simple wots_pk_from_sig_custom takes %ld cycles in HW/SW co-design\n", end - start);
