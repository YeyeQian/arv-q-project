 ...
 */
static void wots_gen_sk_custom(unsigned char *sk, const unsigned char *sk_seed,
                        uint32_t wots_addr[8])
{
    ...
    /* Generate sk element. */
    prf_addr_custom(sk, sk_seed, wots_addr);
}
 ...
 */
static void gen_chain_custom(unsigned char *out, const unsigned char *in,
                      ...
                      const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    for (i = start; i < (start+steps) && i < SPX_WOTS_W; i++) {
        ...
        thash_custom(out, out, 1, pub_seed, addr);
    }
}
 ...
 */
void wots_gen_pk_custom(unsigned char *pk, const unsigned char *sk_seed,
                 const unsigned char *pub_seed, uint32_t addr[8])
{
    uint32_t i;
    ...
    for (i = 0; i < SPX_WOTS_LEN; i++) {
        ...
        wots_gen_sk_custom(pk + i*SPX_N, sk_seed, addr);
        gen_chain_custom(pk + i*SPX_N, pk + i*SPX_N,
 ...
 */
void wots_sign_custom(unsigned char *sig, const unsigned char *msg,
               ...
               uint32_t addr[8])
{
    ...
    for (i = 0; i < SPX_WOTS_LEN; i++) {
        set_chain_addr(addr, i);
        wots_gen_sk_custom(sig + i*SPX_N, sk_seed, addr);
        gen_chain_custom(sig + i*SPX_N, sig + i*SPX_N, 0, lengths[i], pub_seed, addr);
    }
}
 ...
 * Writes the computed public key to 'pk'.
 */
void wots_pk_from_sig_custom(unsigned char *pk,
                      ...
                      const unsigned char *pub_seed, uint32_t addr[8])
{
    ...
    for (i = 0; i < SPX_WOTS_LEN; i++) {
        set_chain_addr(addr, i);
        gen_chain_custom(pk + i*SPX_N, sig + i*SPX_N,
