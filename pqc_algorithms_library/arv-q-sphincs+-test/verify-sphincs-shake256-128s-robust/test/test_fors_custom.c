...
bool fors_custom_profiling()
{
    ...
    printf("sphincs-shake256-128s-robust fors_sign_custom takes %ld cycles in HW/SW co-design\n", end - start);

    printf("begin fors_pk_from_sig_custom for profiling\n");
    start = read_cycle();
    fors_pk_from_sig_custom(pk2, sig, m, pub_seed, addr);
    end = read_cycle();
    printf("sphincs-shake256-128s-robust fors_pk_from_sig_custom takes %ld cycles in HW/SW co-design\n", end - start);
