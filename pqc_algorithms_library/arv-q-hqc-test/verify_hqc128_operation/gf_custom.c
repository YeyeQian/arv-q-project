...
/**********************************************************************
 *                Using gfmul to realize gfinv
**********************************************************************/

uint16_t gf_inverse_custom(uint16_t a) {
    vuint16m1_t va,vb,vc,vd;
    size_t vl;
    size_t avl=1;
    csr_primpoly_rw(PARAM_GF_POLY);
    vl=vsetvl_e16m1(avl);
    va=vmv_s_x_u16m1(va,a,vl);

    vb=vgfmul_vv_u16m1(va,va);//inv = gf_square(a); /* a^2 */
    vc=vgfmul_vv_u16m1(vb,va);//tmp1 = gf_mul(inv, a); /* a^3 */
    vb=vgfmul_vv_u16m1(vb,vb);//inv = gf_square(inv); /* a^4 */
    vd=vgfmul_vv_u16m1(vb,vc);//tmp2 = gf_mul(inv, tmp1); /* a^7 */
    vc=vgfmul_vv_u16m1(vb,vd);//tmp1 = gf_mul(inv, tmp2); /* a^11 */
    vb=vgfmul_vv_u16m1(vc,vb);//inv = gf_mul(tmp1, inv); /* a^15 */
    vb=vgfmul_vv_u16m1(vb,vb);//inv = gf_square(inv); /* a^30 */
    vb=vgfmul_vv_u16m1(vb,vb);//inv = gf_square(inv); /* a^60 */
    vb=vgfmul_vv_u16m1(vb,vb);//inv = gf_square(inv); /* a^120 */
    vb=vgfmul_vv_u16m1(vb,vd);//inv = gf_mul(inv, tmp2); /* a^127 */
    vb=vgfmul_vv_u16m1(vb,vb);//inv = gf_square(inv); /* a^254 */

    uint16_t inv;
    inv=vmv_x_s_u16m1_u16(vb);
    return inv;
}