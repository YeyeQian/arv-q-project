...
void cyclic_shift_left(uint8_t res_out[VEC_N_SIZE_BYTES], const uint8_t dense_padded[VEC_N_SIZE_BYTES+1], const uint32_t offset) {
    ...
    uint32_t bit_shift2 = bit_shift1 + 8 - (PARAM_N & (8 - 1));
    ...
    while (chunk_solved < VEC_N_SIZE_BYTES) {
        uint32_t len = (chunk_solved + 32) <= VEC_N_SIZE_BYTES ? 32 : (VEC_N_SIZE_BYTES - chunk_solved);
        uint32_t temp = chunk_solved + chunk_shift;
        bool flag1 = temp >= VEC_N_SIZE_BYTES;
        uint32_t bunch_offset = flag1 ? (temp - VEC_N_SIZE_BYTES) : temp;
        //loop2: simulate vector processing on a bunch of chunk
        for (int j = 0; j < len; j++) {
            uint32_t temp = bunch_offset + j;
            bool flag2 = temp >= VEC_N_SIZE_BYTES;
            uint32_t chunk_offset = flag2 ? temp - VEC_N_SIZE_BYTES : temp;
    ...
    uint8_t final_chunk_mask = ((1ULL << (PARAM_N & (8 - 1))) - 1);
    res_out[VEC_N_SIZE_BYTES - 1] &= final_chunk_mask;
}

void vect_mul_cycshift(uint64_t* out, const uint64_t* dense, const uint32_t* support, const uint32_t weight) {
    uint8_t dense_padded[VEC_N_SIZE_BYTES + 1] = { 0 };
    memcpy(dense_padded+1,(uint8_t*)dense,VEC_N_SIZE_BYTES);
    uint8_t additional_chunk = 0;
    uint8_t last_chunk_bits = PARAM_N & (8 - 1);
    additional_chunk = (((uint16_t)dense_padded[VEC_N_SIZE_BYTES] << 8 | (uint16_t)dense_padded[VEC_N_SIZE_BYTES - 1]) >> last_chunk_bits) & 255;
    dense_padded[0] = additional_chunk;

    for (int i = 0; i < weight; i++) {
        cyclic_shift_left((uint8_t*)out,dense_padded,support[i]);
    }
}

void cyclic_shift_left_in32(uint32_t res_out[VEC_N_SIZE_64*2], const uint32_t dense_padded[VEC_N_SIZE_64*2+1], const uint32_t offset) {
    uint32_t chunk_mask = 0xffffffff;
    uint32_t chunk_solved = 0; //track the number of chunk already dealt with
	uint32_t chunk_shift = offset >> 5;
	uint32_t bit_shift1 = offset & (32 - 1);
	uint32_t bit_shift2 = bit_shift1 + 32 - (PARAM_N & (32 - 1));
	bool flag0 = bit_shift2 > 32;
	//loop1: do stripmining on a_in, each round of loop deal with 16(VLMAX) chunk and there are 386(R_SIZE_) chunks
	while (chunk_solved < (VEC_N_SIZE_64*2)) {
		uint32_t len = (chunk_solved + 32) <= VEC_N_SIZE_64*2 ? 32 : (VEC_N_SIZE_64*2 - chunk_solved);
		uint32_t temp = chunk_solved + chunk_shift;
		bool flag1 = temp >= VEC_N_SIZE_64*2;
		uint32_t bunch_offset = flag1 ? (temp - VEC_N_SIZE_64*2) : temp;
		//loop2: simulate vector processing on a bunch of chunk
		for (int j = 0; j < len; j++) {
			uint32_t temp = bunch_offset + j;
			bool flag2 = temp >= VEC_N_SIZE_64*2;
			uint32_t chunk_offset = flag2 ? temp - VEC_N_SIZE_64*2 : temp;
			uint32_t bit_shift = flag1 || flag2 ? (flag0 ? bit_shift2 - 32 : bit_shift2) : bit_shift1;
			int32_t sp_offset = (flag1 || flag2) && flag0 ? -1 : 0;
			uint32_t chunk_shifted = ((((uint64_t)dense_padded[chunk_solved + j + 1 + sp_offset] << 32 | (uint64_t)dense_padded[chunk_solved + j + sp_offset]) << bit_shift) >> 32) & chunk_mask;
			res_out[chunk_offset] ^= chunk_shifted;
		}
		chunk_solved += len;
	}
	uint32_t final_chunk_mask = ((1ULL << (PARAM_N & (32 - 1))) - 1);;
	res_out[VEC_N_SIZE_64*2 - 1] &= final_chunk_mask;
}

void vect_mul_cycshift_in32(uint64_t* out, const uint64_t* dense, const uint32_t* support, const uint32_t weight) {
    uint32_t dense_padded[VEC_N_SIZE_64*2 + 1] = { 0 };
    memcpy(dense_padded+4,(uint8_t*)dense,VEC_N_SIZE_BYTES);

    uint32_t additional_chunk = 0;
    uint32_t last_chunk_bits = PARAM_N & (32 - 1);
    additional_chunk = (((uint64_t)dense_padded[VEC_N_SIZE_64*2] << 32 | (uint64_t)dense_padded[VEC_N_SIZE_64*2 - 1]) >> last_chunk_bits) & 0xffffffff;
    dense_padded[0] = additional_chunk;

    for (int i = 0; i < weight; i++) {
        cyclic_shift_left_in32((uint32_t*)out,dense_padded,support[i]);
    }
}