...
void hadamard_redundant(expandedCodeword *src, expandedCodeword *dst) {
    // the passes move data:
    // src -> dst -> src -> dst -> src -> dst -> src -> dst
    // using p1 and p2 alternately
    expandedCodeword *p1 = src;
...
bool test_hadamard_custom(){
    ...
    expandedCodeword expanded_res;
    expand_and_sum(&expanded_ref, &codeArray[0]);
    expand_and_sum(&expanded_res, &codeArray[0]);

    uint64_t start, end;
    expandedCodeword transform_ref;
    expandedCodeword transform_res;

    start=read_cycle();
    hadamard(&expanded_ref, &transform_ref);
    end=read_cycle();
    printf("Reference finished with %lu cycles\n",end-start);

    start=read_cycle();
    hadamard_custom(&expanded_res, &transform_res);
    end=read_cycle();
    printf("Custom finished with %lu cycles\n",end-start);
    ...
    bool flag=true;
    for(int i=0;i<128;i++){
        if(transform_ref[i]!=transform_res[i]){
            flag=false;
            break;
            // printf("ref[%d]=%lu,res[%d]=%lu\n",i,expanded_ref[i],i,expanded_res[i]);
        }
    ...
    printf("test_hadamard_custom return with flag %d\n",flag);

    return flag;
}

bool test_hadamard_redundant(){
    codeword codeArray[MULTIPLICITY];
    srand((unsigned)time(NULL));
    for(int i=0;i<MULTIPLICITY;i++){
        for(int j=0;j<4;j++){
    ...
    uint8_t buffer[128];
    shake256_custom(buffer,128,seed,32);
    ...
    start=read_cycle();
    hadamard_redundant(&expanded_res, &transform_res);
    end=read_cycle();
    printf("Redundant finished with %lu cycles\n",end-start);
    ...
    printf("test_hadamard_redundant return with flag %d\n",flag);
...
//     // test_hadamard_custom();
//     test_hadamard_redundant();
